...
1. **Startup** — `main.ts` computes `MBA_DIR` from `MBA_DIR` or defaults to the repository root `.MBA/` next to `.Manual/`. `server.ts` uses the same default unless an explicit `mbaDir` is passed in tests.
2. **Model identity discovery** — The proxy already probes the upstream server (`/legacy/dna`, or `/v1/models` + `/props`) for the `{digest, quant, build}` tuple used by KV block caching. That same tuple is now shared with MBA resolution via a mutable holder, so DNA-pinned adapters match once the probe completes.
...
8. **Fallback** — If `.MBA/adapters` does not exist, the proxy falls back to the legacy `.legacy-store/bcb/tool-circuit-breakers.json` path.
...
- Adapter YAML files must use `apiVersion: mba.ai/v1alpha1`. Older or unknown versions are rejected.
