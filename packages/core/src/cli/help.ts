...
export function usageOverview(): string {
  return [
    ...
    "",
    heading("Groups"),
    cmd("mba models", "list, edit, pull  (m)"),
    cmd("mba servers", "list, boot, stop, logs  (s)"),
...
export function usageModels(): string {
  return [
    `${brand("models")}`,
    "",
    cmd("mba models", "pick and edit (list when piped)"),
...
export function usageServers(): string {
  return [
    `${brand("servers")}`,
    "",
    cmd("mba servers", "list / boot / stop (TTY menu)"),
    ...
    "",
    dim("  mba servers               list / boot / stop on a TTY"),
...
export function usageMachine(): string {
  return [
    `${brand("machine")}`,
    "",
    cmd("mba machine", "print current mode"),
