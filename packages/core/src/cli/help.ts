...
export function usageOverview(): string {
  return [
    ...
    "",
    heading("Daemon"),
    cmd("mba start", "run the daemon in the background"),
    cmd("mba stop", "stop the daemon"),
    cmd("mba restart", "stop then start (picks up a rebuild)"),
    "",
    heading("Groups"),
    ...
    "",
    dim("  mba start --foreground   this terminal (not restart)"),
