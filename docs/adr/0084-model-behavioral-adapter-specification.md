...
Model behavior varies by weights, quantization, inference server, client harness, and IDE. The same `read_file` loop breaker that helps a weak local model may be unnecessary or counter-productive for a strong cloud model, and a grammar that works under llama.cpp may fail under vLLM. Today these differences are either hard-coded in the original project or inferred at request time from the harness/model catalog key, not from a first-class model description.

We need a portable, consumer-agnostic way to say: *"For this model (or family), when running in this environment on this server, here are the behavioral rules and output-shape preferences."* the original project should consume this specification, not own it.
...
- Make the format reusable by tools other than the original project.
- Preserve live-editable JSONL rule bindings that map to "if this happens, do/say that."

## Considered Options

- **Option A — the original project-native config extension:** extend the existing `.legacy-store/bcb/tool-circuit-breakers.json` with per-model sections. Fast to implement, but couples the format to the original project internals and does not solve environment/server granularity.
- **Option B — Standalone MBA spec with YAML index + JSONL bindings:** define an independent Model Behavioral Adapter format. the original project reads and maps it to its own rule implementations. Slightly more design work up front, but portable and cleanly layered.
...
- The MBA spec can be consumed by the original project and by other tools that need model-behavioral configuration.
...
- One-time migration of the existing global BCB config from `.legacy-store/bcb/` to `.MBA/bcb/`, with a temporary legacy-path fallback in the consumer until the move completes.
...
Event names are dot-separated and consumer-defined. Examples in the original project today include `tcb:tripped`, `tcb:clamped`, `tcb:hint`, and `tcb:killed`. The spec additionally reserves `mba:*` events emitted by the resolution/loading machinery itself: `mba:ambiguous-resolution`, `mba:adapter-upgrade-deferred`, and `mba:load-error`.
...
All model-behavioral configuration — adapters and the global BCB/TCB config — consolidates under a single `.MBA/` root, replacing the current split across `.legacy-store/`:

```text
.MBA/
├── bcb/
│   └── tool-circuit-breakers.json   # global fallback layer (moved from .legacy-store/bcb/)
└── adapters/
...
Migration: the consumer looks in `.MBA/` first and falls back to the legacy `.legacy-store/bcb/` path until the move lands; the fallback is removed once migration completes. Exact layout is a consumer convention; the MBA spec only requires that bindings are resolvable relative paths.

## Pros and Cons of the Options

### Option A — the original project-native config extension

- ✅ Fewer new abstractions; can reuse existing `ToolCircuitBreakerConfig` types directly.
- ❌ Ties the format to the original project internals; not reusable by other tools.
