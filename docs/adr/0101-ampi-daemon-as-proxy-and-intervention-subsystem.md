...
### 3. The original proxy's fate

The original proxy loses its TCB/escalation/AMPI role. Its remaining responsibilities (grammar injection, KV cache management, SSE streaming, and other model-serving concerns) are either:

- absorbed into the MBA daemon, or
- retained in a thin original proxy that delegates all model-behavior logic to the MBA daemon.

The exact split is a migration detail (see Migration Staging below), not an architectural commitment. The commitment is: **no model-behavior logic (TCB, escalation, AMPI) remains in the original proxy.**
...
- **Biggest structural commitment yet.** The daemon becomes the proxy. This is a re-architecture of the request path, not a feature addition. The original proxy's model-behavior logic must be migrated out.
...
- **Migration risk.** The original proxy's request path depends on the TCB/escalation logic being moved. A bad migration breaks the live proxy. Mitigation: staged migration (see below), with the original proxy retaining a thin pass-through until the daemon is proven.
...
6. **original proxy's remaining responsibilities** — which model-serving concerns (grammar, KV, SSE) stay in a thin original proxy vs. are absorbed into the MBA daemon? This is a migration detail, not an architectural commitment.
